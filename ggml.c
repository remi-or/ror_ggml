...
/// [int8_a, int8_b, int8_c, int8_d, ...], [int8_A, int8_b, int8_C, int8_D, ...] -> 
/// [int16_(aA+bB), int16_(cC+dD), ...] -> [fp32_(aA+bB+cC+dD)]
...
typedef struct {
    ggml_fp16_t d;         // delta
    ggml_fp16_t m;         // min
    uint8_t qs[QK4_3 / 2]; // nibbles / quants /// qs contains QK4_3=16 four_bit fields stored as 8 8_bit fields
} block_q4_3; // La structure qui nous intéresse
...
typedef struct {
    ...
    /// qs -> pointeur de 32 int8_t (c'est signed char, donc -128 à 127) qui en sont vraiment
} block_q8_0;
...
/// Function to quantize a block into the block_q4_3
static void quantize_row_q4_3_reference(const float * restrict x, block_q4_3 * restrict y, int k) {
    ...
    for (int i = 0; i < nb; i++) {
        ...
        const float d = (max - min) / ((1 << 4) - 1); /// block_q4_3.d = (max-min) / 15
        const float id = d ? 1.0f/d : 0.0f; /// id = 1/d si d != 0

        y[i].d = GGML_FP32_TO_FP16(d);
        y[i].m = GGML_FP32_TO_FP16(min); /// block_q4_3.m = min

        for (int l = 0; l < QK4_3; l += 2) {
            const float v0 = (x[i*QK4_3 + l + 0] - min)*id; /// left normalized value
            const float v1 = (x[i*QK4_3 + l + 1] - min)*id; /// rght normalized value

            const uint8_t vi0 = (int) (v0 + 0.5f); /// rounding operation
            const uint8_t vi1 = (int) (v1 + 0.5f); 

            assert(vi0 < 16); /// should be between 0 and 15
            assert(vi1 < 16);

            y[i].qs[l/2] = vi0 | (vi1 << 4); // qs[0] = left_quantized @ right_quantized
            /// this means that qs contains QK4_3=16 four_bit fields
        }
    }
}
...
// reference implementation for deterministic creation of model files
/// function to create q8_0 blocks, of size 32 it seems
static void quantize_row_q8_0_reference(const float * restrict x, block_q8_0 * restrict y, int k) {
    ...
    for (int i = 0; i < nb; i++) {
        ...
        /// we scan the QK8_0 = 32 next float values and retains the max absolute value
        for (int l = 0; l < QK8_0; l++) {
            const float v = x[i*QK8_0 + l]; 
            amax = MAX(amax, fabsf(v));
        }

        const float d = amax / ((1 << 7) - 1); /// d is max abs value / 127
        const float id = d ? 1.0f/d : 0.0f; /// id = inverse of d
        ...
        for (int l = 0; l < QK8_0/2; ++l) {
            const float v0 = x[i*QK8_0           + l]*id; /// spans x_j/d for j in i*QK8_0 + ( 0,  1,  2, ..., 15)]
            const float v1 = x[i*QK8_0 + QK8_0/2 + l]*id; /// spans x_j/d for j in i*QK8_0 + (16, 17, 18, ..., 31)
            /// both should be between -127 and 127

            y[i].qs[          l] = roundf(v0); /// ys[i].qs = [x_0/d, ..., x_15/d]
            y[i].qs[QK8_0/2 + l] = roundf(v1); /// ys[i].qs = [x_16/d, ..., x_31/d]

            sum0 += y[i].qs[          l]; /// sum of [x_0, ..., x_15]/d
            sum1 += y[i].qs[QK8_0/2 + l];
        }

        y[i].s0 = d * sum0; /// sum of [x_0, ..., x_15]
        y[i].s1 = d * sum1;
    }
}
...
static void ggml_vec_dot_q4_3_q8_0(const int n, float * restrict s, const void * restrict vx, const void * restrict vy) {
    ...
    *s = vaddvq_f32(vaddq_f32(sumv0, sumv1)) + summs0 + summs1;
    ...
    for (int i = 0; i < nb; i++) {
        ...
        /// nibbles is x[2*i + 0].qs = [left_int4, right_int4]
        /// bytes_from_nibbles return [nibble0, ..., nibble_15] where each nibble is an int4
        const __m128i bx0 = bytes_from_nibbles_16(x[2*i + 0].qs); /// [int8_nx0, ..., int8_nx15]
        const __m128i bx1 = bytes_from_nibbles_16(x[2*i + 1].qs); /// [int8_nx16, ..., int8_nx31]
        const __m256i bx = _mm256_set_m128i(bx1, bx0);
        /// bx = [int8_nx16, ..., int8_nx31, int8_nx0, ..., int8_nx15] so 32 int8 (in 0...15 range so sign or not)

        const __m256 dy = _mm256_broadcast_ss(&y[i].d); /// le fp32 delta_y broadcast 8 fois dans un vecteur de 8*32=256 bit
...
// New function : 1st term computation main function
...
// New function: x2w2 sum main function
...
size_t ggml_quantize_q4_3(const float * src, void * dst, int n, int k, int64_t * hist) {
    assert(k % QK4_3 == 0); // Assert the size of what is quantized is a multiple of 16
    const int nb = k / QK4_3; // nb = number of values to be quantized
